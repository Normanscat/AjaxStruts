...
<div class="content">
	...
	<table>
		...
		<tbody>
			<!-- varStatus获取当前索引或序号 -->
			<c:forEach var="item" items="${juri }" varStatus="status">
				<tr class="goods-content-item" data-id="${item.id }">
					...
					<td><a class="edit" data-id="${item.id }"><u>编辑</u></a> <a
						class="juridelete" data-id="${item.id }"><u>删除</u> </a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<script>
	$(function() {
		...
		$(".edit").on("click", function() {
			var $content = $(".content")
			var $this = $(this);
			var jurieditId = $this.data("id");
			$content.load("editjuri", {
				jurieditId : jurieditId
			});
		});
		//添加
		$(".btn-add-panel").on("click", function() {
			var $content = $(".content");
			$content.load("addjuri")
		});
		//批量删除
